...
Meteor.publish("profileTopics", function(user) {
  var comments = Comments.find({"owner": user}).fetch(), result = [];
  _.each(comments, function(comment) {
    result.push(comment.topic);
  });
  return Topics.find({$or: [{"owner": user}, {"_id": {$in: result}}]}, {fields: {"_id": 1, "title": 1}});
});
Meteor.publish("profileComments", function(user) { //publish by owner rather than by topic
  ...
  return Comments.find({$or: [{"owner": user}, {"_id": {$in: liked}}]}, {fields: {"_id": 1, "owner": 1, "content": 1, "topic": 1}});
});
// Meteor.publish("profileCommentsByUser", function(user, limit) {
//  return Comments.find({"owner": user}, {limit: limit, fields: {"_id": 1, "content": 1, "topic": 1}});
// });
// Meteor.publish("profileCommentsByLiked", function(user, limit) { //publish by owner rather than by topic
//  var liked = Meteor.users.findOne({"username": user}).activity.liked;

//  return Comments.find({"_id": {$in: liked}}, {limit: limit, fields: {"_id": 1, "owner": 1, "content": 1, "topic": 1}});
// });