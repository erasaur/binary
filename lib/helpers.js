...
getComments = function (selector) {
  var controller = getCurrentController();
  if (!Meteor.userId() || !controller) return;

  var runAt = controller._runAt;
  selector.createdAt = { $lt: runAt };
  return Comments.find(selector, { sort: { 'score': -1 } }).fetch();
};
...
getCurrentQuery = function () {
  var controller = getCurrentController();
  return controller.params && controller.params.query;
};
getCurrentHash = function () {
  var controller = getCurrentController();
  return controller.params && controller.params.hash;
};
