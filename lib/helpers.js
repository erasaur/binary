...
getCommentRoute = function (topicId, commentId) {
  return Router.routes['comment'].path({
    '_id': topicId,
    'commentId': commentId
  });
};
getCommentUrl = function (topicId, commentId) {
  return Router.routes['comment'].path({
    '_id': topicId,
    'commentId': commentId
  });
...
markdownToHTML = function (s) {
  return typeof s === 'string' && sanitize(marked(s));
};

// Strips any form of whitespace, including spaces and newlines
stripSpaces = function (s) {
  return s && s.replace(/\s|&nbsp;/gm, '');
};
